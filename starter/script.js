    ...
    if (searchItem === 'Friends') {
      ...
      if (friendName === 'Ron') {
        ...
        if (hermoineLocation === 'Aritmancy Class') {
          ...
          let hermoineGryffindor = confirm('Hermoine is not here. Please return to beginning')
        ...
        if (nevilleLocation === 'Herbology Class') {
          ...
          let nevilleEnd = confirm('Neville cannot be reached in this location. Please return to start')
          if (nevilleEnd) {
            game()
          } 
        }
      ...
      switch (horcruxInt) {
        case 3: 
          let horcruxResult = prompt ('What horcrux are you searching for, master Potter?')
          switch (horcruxResult) {
            case 'Nagini':
              let naginiLocation = prompt('Where would you like to search for Nagini, Master Potter?') 
              switch (naginiLocation) {
                case 'Little Rangleton Cemetary': 
                  let rangletonValue = confirm('Nangini slithered away. Please return to start')
                  if (rangletonValue) {
                    game()
                  }
                  break;
                case 'Hogwarts': 
                  ...
                  break;
                default: 
                  let noNagini = confirm('Nagini cannot be reached in this location. Please return to start')
                  if (noNagini) {
                    game()
                  } 
              }
            ...
            case "Tom Riddle's Diary":
              let riddleLocation = prompt("Where would you like to search for Riddle's diary, Master Potter?")
              switch (riddleLocation) {
                case 'library':
                  let libraryLocation = confirm("Tom Riddle's diary has been moved. Please return to start")
                  if (libraryLocation) {
                    game()
              }
                  break;
                case 'Chamber of Secret':
                  let chamberLocation = confirm('The diary was already destroyed in the Chamber of Secret')
                  if (chamberLocation) {
                    game()
                  }
                  break;
              break;
            case 'Secret Horcrux':
              let secretLocation = prompt('Where do you think the secret horcrux is, Mater Potter?')
              break;
            default:
          }
          break;
